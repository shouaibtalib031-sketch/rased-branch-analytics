...
export async function initDatabase() {
  const schema=`
    ...
    create table if not exists observations(id text primary key,visit_id text references visits(id) on delete cascade,item_id text references operational_items(id),category text,body text,severity text,state text default 'new',is_repeated boolean default false,repetition_count int default 1,assignee_id text references users(id),due_at timestamptz,closed_at timestamptz,created_at timestamptz default now());
    ...
    create table if not exists interventions(id text primary key,branch_id text references branches(id),visit_id text references visits(id),report_id text references reports(id),observation_id text references observations(id),reason text,severity text,intervention_type text,status text default 'open',created_at timestamptz default now());
    create index if not exists interventions_branch_idx on interventions(branch_id,created_at desc);
  `;
  ...
  await db.query("alter table observations add column if not exists item_id text references operational_items(id)");
  await db.query("alter table observations add column if not exists report_id text references reports(id)");
  await db.query("alter table observations add column if not exists branch_id text references branches(id)");
  await db.query("alter table observations add column if not exists followup_created boolean default false");
  await db.query("alter table reports add column if not exists original_filename text");
  await db.query("alter table reports add column if not exists stored_filename text");
  await db.query("alter table reports add column if not exists file_size bigint");
  await db.query("alter table reports add column if not exists upload_status text default 'uploaded'");
  await db.query("alter table reports add column if not exists analysis_status text default 'uploaded'");
  await db.query("alter table reports add column if not exists analysis_error text");
  await db.query("alter table reports add column if not exists branch_id text references branches(id)");
  await db.query("alter table reports add column if not exists visit_id text references visits(id)");
  await db.query("alter table reports add column if not exists supervisor_id text references supervisors(id)");
  await db.query("alter table reports add column if not exists uploaded_at timestamptz default now()");
  await db.query("alter table reports add column if not exists analysis_started_at timestamptz");
  await db.query("alter table reports add column if not exists analysis_completed_at timestamptz");
  ...
  await db.query("create index if not exists observations_item_idx on observations(item_id)");
  await db.query("create index if not exists observations_report_idx on observations(report_id)");
  await db.query("create index if not exists observations_branch_idx on observations(branch_id)");
  await db.query("create index if not exists reports_analysis_status_idx on reports(analysis_status,uploaded_at desc)");
  await db.query("update reports set original_filename=coalesce(original_filename,original_name), file_size=coalesce(file_size,size_bytes), uploaded_at=coalesce(uploaded_at,created_at), stored_filename=coalesce(stored_filename,storage_path), upload_status=coalesce(upload_status,'uploaded'), analysis_status=coalesce(analysis_status,case when status in ('approved','analyzed','completed') then 'completed' when status='failed' then 'failed' else coalesce(status,'uploaded') end)");
  await db.query("update reports r set branch_id=coalesce(r.branch_id,v.branch_id), visit_id=coalesce(r.visit_id,v.id), supervisor_id=coalesce(r.supervisor_id,v.supervisor_id) from visits v where v.report_id=r.id");
  await db.query("update observations o set report_id=coalesce(o.report_id,v.report_id), branch_id=coalesce(o.branch_id,v.branch_id) from visits v where v.id=o.visit_id");
  await seed();
